import React from 'react'
import "./Navbar.css"
import { useState } from "react";
import { useNavigate } from 'react-router-dom';
//import logo from "../data/data.json"
const searchIcon = <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#000000"><path d="M0 0h24v24H0V0z" fill="none" /><path d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z" /></svg>
const logo = "https://i01.appmifile.com/webfile/globalimg/pandora/mi-logo.svg";

const Navbar = () => {




    const [keyword, setKeyword] = useState("");
    const navigate = useNavigate();

    const searchSubmitHandler = (e) => {
        e.preventDefault();
        if (keyword.trim()) {//all space remove
            navigate(`/products/${keyword}`);
        } else {
            navigate("/products");
        }
    };

    return (
        <>
            <div className="nav">
                <div className="logo">

                    <a href="/">
                        <img id="logoImage" src={logo} alt="Not" />
                    </a>

                </div>

                <a className="navlinks" href="/#miphones">Elecronics</a>
                <a className="navlinks" href="/#miphones">Mobole Phones</a>

                <a className="navlinks" href="/#tv">Grocery</a>
                <a className="navlinks" href="/#laptops">Laptops</a>
                <a className="navlinks" href="/#lifestyle">Fitness & Lifestyle</a>
                <a className="navlinks" href="/#home">Fasion</a>
                <a className="navlinks" href="/#audio">Beauty</a>
                <a className="navlinks" href="/#accessories">Accessories</a>

                <div className="searchbox">
                    <input type="text" name="search" placeholder="Search Products" onChange={(e) => setKeyword(e.target.value)} />
                    {/* <input type="submit" value="Search" onClick={searchSubmitHandler} /> */}
                    {/* {searchIcon} */}

                    {/* <a href="/products/${keyword}" >{searchIcon}</a> */}
                    <span onClick={searchSubmitHandler}>{searchIcon}</span>

                </div>
                {/* <div className="searchBox">
                    <form onSubmit={searchSubmitHandler}>
                        <input type="text" name="search" placeholder="Search Products" />
                        {searchIcon}
                        <input type="submit" value="Search" />
                    </form>
                </div> */}


            </div>
        </>
    )
}

export default Navbar
